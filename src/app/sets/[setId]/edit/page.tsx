...
export default async function EditSetPage({ params }: { params: Promise<{ setId: string }> }) {
  ...
  const { data: set } = await supabase
    .from("word_sets")
    .select("id,title,description,grade_min,grade_max")
    .eq("id", setId)
  ...
  return (
    <main className="min-h-screen bg-muted/35">
      ...
      <div className="mx-auto max-w-6xl px-4 py-8 sm:px-6 sm:py-10">
        ...
        <CreateSetForm initialSet={{ id: set.id, title: set.title, grade, words: cards.map((card) => card.word_text) }} />
      </div>
    </main>
  );
}
