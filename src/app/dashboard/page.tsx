...
export default async function DashboardPage() {
  ...
  return (
    <main className="min-h-screen bg-muted/35">
      <header className="border-b bg-background"><div className="mx-auto flex min-h-16 max-w-7xl items-center justify-between gap-3 px-4 py-3 sm:px-6"><BrandLogo /><div className="flex items-center gap-2">{account.access?.role === "admin" && <Button asChild variant="outline" size="sm"><Link href="/admin/users"><ShieldCheck className="mr-2 size-4" />จัดการผู้ใช้</Link></Button>}<Button asChild variant="ghost" size="icon"><Link href="/settings" aria-label="ตั้งค่าบัญชี"><Settings className="size-4" /></Link></Button><Avatar className="size-9"><AvatarFallback>{initials}</AvatarFallback></Avatar><form action={logoutAction}><Button type="submit" variant="ghost" size="icon" aria-label="ออกจากระบบ"><LogOut className="size-4" /></Button></form></div></div></header>
      <div className="mx-auto max-w-7xl px-4 py-8 sm:px-6 sm:py-10">
        ...
        <Card className="mt-8"><CardHeader className="border-b"><CardTitle>ชุดคำของฉัน</CardTitle></CardHeader><CardContent className="p-5">{!sets?.length ? <div className="py-12 text-center"><BookOpen className="mx-auto size-10 text-muted-foreground" /><h2 className="mt-4 font-semibold">ยังไม่มีชุดคำ</h2><p className="mt-1 text-sm text-muted-foreground">สร้างชุดแรก แล้วระบบจะเก็บไว้ในโปรไฟล์ของคุณ</p><Button asChild className="mt-5"><Link href="/create">สร้างชุดคำ</Link></Button></div> : <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">{sets.map((set) => <article key={set.id} className="overflow-hidden rounded-2xl border bg-background"><div className="grid h-28 place-items-center bg-blue-600 text-3xl font-bold text-white">ก ข</div><div className="p-4"><div className="flex items-start justify-between gap-2"><div><h2 className="font-semibold">{set.title}</h2><p className="mt-1 text-sm text-muted-foreground">{set.word_cards?.[0]?.count ?? 0} คำ</p></div><Badge variant={set.is_published ? "default" : "secondary"}>{set.is_published ? "สาธารณะ" : "ส่วนตัว"}</Badge></div><div className="mt-4 grid grid-cols-2 gap-2"><Button asChild><Link href={`/play/${set.public_slug}`}><Play className="mr-1 size-4" />เล่น</Link></Button><Button asChild variant="outline"><Link href={`/play/${set.public_slug}`} target="_blank"><ExternalLink className="mr-1 size-4" />เปิดลิงก์</Link></Button></div></div></article>)}</div>}</CardContent></Card>
      </div>
    </main>
  );
}
