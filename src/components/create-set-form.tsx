...
import { createWordSetAction } from "@/app/create/actions";
...
export function CreateSetForm() {
  ...
  function save() {
    ...
    startTransition(async () => {
      const result = await createWordSetAction({ title, grade, words });
      if (result.error) return setError(result.error);
      router.push(`/play/${result.slug}`);
    });
  }
    ...
    <div className="grid gap-6 lg:grid-cols-[minmax(0,1fr)_360px]">
      <Card>
        ...
        <CardContent className="space-y-7 p-6">
          ...
          <div className="space-y-3"><Label>จำนวนแผ่นป้าย</Label><div className="flex flex-wrap gap-2">{countOptions.map((option) => <Button key={option} type="button" variant={count === option ? "default" : "outline"} className="min-w-14" onClick={() => setCount(option)}>{option}</Button>)}</div></div>
          ...
          <div className="flex flex-col-reverse justify-between gap-3 border-t pt-6 sm:flex-row"><Button type="button" variant="ghost" onClick={() => router.back()}><ArrowLeft className="mr-1 size-4" /> ย้อนกลับ</Button><Button type="button" disabled={!isReady || pending} onClick={save}>{pending ? <LoaderCircle className="mr-2 size-4 animate-spin" /> : <ArrowRight className="mr-1 size-4" />}บันทึกและเริ่มเล่น</Button></div>
        </CardContent>
      </Card>
