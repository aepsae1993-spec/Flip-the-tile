...
export function GameBoard({ title: initialTitle, words: initialWords }: { title?: string; words?: string[] }) {
  ...
  return (
    <main className="min-h-screen bg-sky-50/70">
          ...
          {cards.map((card) => (
            <button key={card.id} type="button" onClick={() => openCard(card.id)} aria-label={card.opened ? `เปิดคำว่า ${card.word}` : `เปิดป้ายหมายเลข ${card.id}`} className={`card-perspective aspect-[4/3] min-h-24 focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-primary/30 ${card.opened ? "card-flipped" : ""}`}>
              <span className="card-inner relative block size-full">
                <span className="card-face absolute inset-0 grid place-items-center overflow-hidden rounded-2xl border-2 border-blue-200 bg-white text-3xl font-bold text-blue-700 shadow-sm transition-[transform,box-shadow] hover:-translate-y-1 hover:shadow-lg sm:text-4xl"><span className="absolute -right-5 -top-5 size-16 rounded-full bg-blue-50" /><span className="relative">{card.id}</span></span>
                <span className={`card-face card-back absolute inset-0 grid place-items-center rounded-2xl px-2 text-center font-bold shadow-lg ${card.status === "correct" ? "bg-emerald-600 text-white" : card.status === "retry" ? "bg-amber-400 text-amber-950" : "bg-primary text-primary-foreground"}`}><span className="text-[clamp(1.1rem,3vw,2rem)] leading-tight">{card.word}</span>{card.status === "correct" && <Check className="absolute right-2 top-2 size-5" />}</span>
              </span>
            </button>
          ))}
        </div>
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent className="max-w-2xl overflow-hidden border-0 p-0 sm:rounded-[2rem]">
          <div className="bg-blue-50 px-6 py-5"><DialogHeader><DialogTitle className="flex items-center justify-center gap-2 text-base text-primary"><Sparkles className="size-4" /> อ่านคำนี้ให้ฟังหน่อย</DialogTitle><DialogDescription className="text-center">ป้ายหมายเลข {activeCard?.id}</DialogDescription></DialogHeader></div>
          <div className="grid min-h-56 place-items-center px-6 py-10"><p className="text-center text-[clamp(3rem,10vw,6rem)] font-bold leading-tight tracking-tight text-blue-950">{activeCard?.word}</p></div>
          <div className="grid grid-cols-2 gap-3 border-t bg-muted/35 p-4 sm:p-5"><Button className="h-12 bg-emerald-600 text-base hover:bg-emerald-700" onClick={() => mark("correct")}><Check className="mr-2 size-5" /> อ่านถูก</Button><Button variant="outline" className="h-12 border-amber-300 bg-amber-50 text-base text-amber-900 hover:bg-amber-100" onClick={() => mark("retry")}><RotateCcw className="mr-2 size-4" /> ลองอีกครั้ง</Button></div>
        </DialogContent>
      </Dialog>
    </main>
  );
